...
def get_connection():
    ...
    conn.execute('PRAGMA foreign_keys = ON;')  # Enable CASCADE on every connection
    return conn
...
def init_db():
    ...
    # Create/Update Cards table with 'tags'
    c.execute('''
    ...
    # Add 'tags' column if it doesn't exist (for existing DBs)
    try:
...
def get_cards_prioritized(deck_id):
    ...
    for row in results:
        ...
        if total_reviews == 0:
            ...
            try:
                last_rev_date = datetime.strptime(last_review_str, '%Y-%m-%d %H:%M:%S')
            except ValueError:
                # Handle SQLite TIMESTAMP varying formats if needed
            ...
            # Simple Spaced Repetition + Weighted Errors heuristic:
            # 1. More days since = higher priority
            # 2. More error rate = higher priority
            # Multiply error_rate to heavily penalize high error cards
...
def get_deck_performance(deck_id):
    ...
    # Average review time (ms), total reviews, correct/incorrect responses
    query = '''
    SELECT 
        COUNT(r.id) as total_reviews,
    ...
    return {
        'global': global_stats, # (total_reviews, correct, incorrect, avg_dur)
        'cards': cards_stats # [(id, front, last_seen), ...]
    }
...
def save_tratak_session(duration_seconds):
    ...
    c = conn.cursor()
    c.execute(
        'INSERT INTO tratak_sessions (duration_seconds) VALUES (?)',
        (duration_seconds,)
    )
    conn.commit()
    conn.close()
